...
func _ready() -> void:
	...
	var enemy_pokemon_list = load_parse_json(pokemon_file_path)["Enemy"]
	
	for pokemon in enemy_pokemon_list:
		var instance = PokemonInstance
		enemy_pokemon_list[pokemon]["name"]
		enemy_pokemon_instances[instance.name] = instance
	
	# Create a signal of enemy pokemon instances and pass it to the combat manager
